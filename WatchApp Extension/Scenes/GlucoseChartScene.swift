...
class GlucoseChartScene: SKScene {
    ...
    private func performUpdate(animated: Bool) {
        ...
        hoursLabel.text = dateFormatter.string(from: window)
        ...
        correctionRange?.quantityBetween(start: start, end: end).forEach({ (range) in
            ...
            sprite.color = UIColor.glucose.withAlphaComponent(activeOverride != nil ? 0.4 : 0.6)
        ...
        if let range = activeOverride {
            ...
            sprite1.color = UIColor.glucose.withAlphaComponent(0.6)
            ...
            if range.end < end {
                ...
                sprite2.color = UIColor.glucose.withAlphaComponent(0.4)
