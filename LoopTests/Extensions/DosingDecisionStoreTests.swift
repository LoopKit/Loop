...
class DosingDecisionStoreCriticalEventLogTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let cacheStore = PersistenceController(directoryURL: URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true).appendingPathComponent(UUID().uuidString, isDirectory: true))
...
fileprivate extension StoredDosingDecision {
    static var test: StoredDosingDecision {
        ...
        let tempBasalRecommendation = TempBasalRecommendation(unitsPerHour: 0.75,
                                                              duration: .minutes(30))
        let automaticDoseRecommendation = StoredDosingDecision.AutomaticDoseRecommendationWithDate(recommendation: AutomaticDoseRecommendation(basalAdjustment: tempBasalRecommendation, bolusUnits: 0), date: dateFormatter.date(from: "2020-05-14T22:38:15Z")!)
